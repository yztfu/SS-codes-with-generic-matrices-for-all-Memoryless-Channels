...
for j in range(11):
    ...
    def integrated_function_out(z,qz_hat):
        ...
        temp1=(0.5+0.5*math.erf(-math.sqrt(2)/2*gamma*z))*(1-epsilon)+epsilon
        temp2=(0.5+0.5*math.erf(-math.sqrt(2)/2*gamma*z))*(1-epsilon)
        if (temp1 == 0):
            temp3 =0
        else:
            temp3 = temp1*math.log(temp1)
        if (temp2 == 0):
            temp4 = 0
        else:
            temp4 = temp2*math.log(temp2)
        return (temp3 + temp4)*math.exp(-z*z/2)/math.sqrt(2*math.pi)

    def I_out(qz_hat):
        return integrate.quad(integrated_function_out,-5,5,args=(qz_hat,))[0]
    ...
    file.write(" ")
    theory = 0.5*alpha*B*(epsilon*math.log(epsilon) + (1-epsilon)*math.log(1-epsilon))
